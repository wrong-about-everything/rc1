...
class SendsArbitraryMessageDuringRoundRegistrationTest extends TestCase
{
    public function testWhenUserDeclinesRoundInvitationThenHisAnswerIsPersistedAndHeSeesSeeYouNextTimeMessage()
    {
    ...
    public function testWhenUserAcceptsRoundInvitationThenHisAnswerIsPersistedAndHeSeesTheSecondQuestion()
    {
        $connection = new ApplicationConnection();
        (new Bot($connection))
            ->insert([
    ...
    public function testWhenUserAnswersTheLastQuestionThenInvitationBecomesAcceptedAndHeBecomesAMeetingParticipantAndSeesCongratulations()
    {
        $connection = new ApplicationConnection();
        (new Bot($connection))
            ->insert([
    ...
    public function testWhenMeetingParticipantSendsArbitraryMessageBeforeRoundStartDateThenHeSeesStatusInfo()
    {
        $connection = new ApplicationConnection();
        (new Bot($connection))
            ->insert([
        ...
        $this->assertCount(1, $transport->sentRequests());
        $this->assertEquals(
            'Вы зарегистрированы на встречу. Я пришлю вам пару 16 июня в 11 утра.\nХотите что-то уточнить? Смело пишите на @gorgonzola_support!',
            (new FromQuery(new FromUrl($transport->sentRequests()[0]->url())))->value()['text']
        );
    }

    public function testWhenMeetingParticipantSendsArbitraryMessageAfterRoundStartDateThenHeSeesStatusInfo()
    {
        $connection = new ApplicationConnection();
        (new Bot($connection))
            ->insert([
        ...
        $this->assertCount(1, $transport->sentRequests());
        $this->assertEquals(
            'Вы зарегистрированы на встречу c @vasya.\nХотите что-то уточнить? Смело пишите на @gorgonzola_support!',
            (new FromQuery(new FromUrl($transport->sentRequests()[0]->url())))->value()['text']
        );
    }
